...
class RSU_Builds {
	...
	/**
	 * Reduce a title or version to a comparable bare string.
	 *
	 * Strips the descriptive "Rivian Software Update" prefix some titles
	 * carry, then lowercases and trims.
	 *
	 * @param string $value Post title or version string.
	 * @return string
	 */
	public static function normalize( $value ) {
		$value = preg_replace( '/^\s*Rivian\s+Software\s+Update\s*/i', '', (string) $value );

		return strtolower( trim( (string) $value ) );
	}

	/**
	 * The release family a build belongs to.
	 *
	 * Rivian's third group encodes platform and patch — 2026.31.00, 2026.31.30
	 * and 2026.31.40 are all the 2026.31 release — so a version with three or
	 * more numeric groups reduces to its first two. Anything else (a bare
	 * "2026.31", or a title that is not a version) comes back normalized but
	 * otherwise untouched.
	 *
	 * @param string $version Version string or title.
	 * @return string
	 */
	public static function family_of( $version ) {
		$version = self::normalize( $version );

		if ( preg_match( '/^(\d{4}\.\d{1,2})(?:\.\d+)+$/', $version, $m ) ) {
			return $m[1];
		}
	...
	/**
	 * Whether a string is a bare release family ("2026.31") rather than a build.
	 *
	 * @param string $value Title or version.
	 * @return bool
	 */
	public static function is_family( $value ) {
		return (bool) preg_match( '/^\d{4}\.\d{1,2}$/', self::normalize( $value ) );
	}

	/**
	 * Whether a string is a full build ("2026.31.00") rather than a family.
	 *
	 * @param string $value Title or version.
	 * @return bool
	 */
	public static function is_build( $value ) {
		return (bool) preg_match( '/^\d{4}\.\d{1,2}(?:\.\d+)+$/', self::normalize( $value ) );
	}

	/**
	 * Find which vehicle/generation slot holds a given build.
	 *
	 * @param array  $builds  `[vehicle][generation] => build`.
	 * @param string $version Build to look for.
	 * @return array|null `array{ vehicle, generation }` or null when absent.
	 */
	public static function find_slot( $builds, $version ) {
		$target = self::normalize( $version );

		if ( '' === $target || ! is_array( $builds ) ) {
			return null;
		}

		foreach ( $builds as $v_slug => $gens ) {
			if ( ! is_array( $gens ) ) {
				continue;
			}
			foreach ( $gens as $g_slug => $build ) {
				if ( self::normalize( $build ) === $target ) {
					return array(
						'vehicle'    => $v_slug,
						'generation' => $g_slug,
					);
